# TALISMAN — MASTER ARCHITECTURE & UPGRADE HANDOFF GUIDE

> **Project Name:** *Talisman* (Philippine Mythology 3D Action MOBA)  
> **Workspace Directory:** `/Users/zer0fx28/talisman`  
> **GitHub Repository:** [`https://github.com/Ipswich38/Talisman`](https://github.com/Ipswich38/Talisman)  
> **Active Branch:** `main`  
...
*Talisman* pits **Dawn** against the **Dusk** across the ancient **Pasig Agimat River** beneath the volcanic haze of **Mount Mayon**.
...
│   │   │   ├── bosses.ts               # Maw, Treant & Pushing Treant AI
│   │   │   ├── brute.ts                # Treant brute duel combat
...
│   │   │   ├── catalogue.ts            # Roster: Veer, Hollow, Willow, Bernardo, etc.
...
│   │       ├── bosses.ts               # Maw & Treant 3D Monster Hunter models
│   │       ├── camps.ts                # Jungle camp props & Idol altars
...
│   │       ├── creeps.ts               # Jungle creep 3D meshes (Hollow, Veer)
...
- **Audio Catalog:**
  1. `playMeleeHit()`: Metallic blade clashing with white noise burst and resonant decay (Blade).
  2. `playPunchHit()`: Low-frequency sub thump with quick pitch drop (Veer stomp / Treant fist).
  3. `playSpellCast(type)`: High harmonic sine sweeps (Lunar beam, Wisp cold fire, Viscera lash).
...
- **Components:**
  ...
  2. **Telegraph Meshes (`reticles.ts`):**
     - **Line / Arrow Reticle:** Rectangular ground projector with forward chevron for skillshots (`Bernardo Slab`, `Argent Beam`, `Wisp`).
     - **Cone Sector Reticle:** Dynamic arc sector mesh showing exact angle and reach for cone abilities (`Veer Stomp`, `Hollow Blood Arc`, `Maw Wave`).
     - **Ground Target Circle:** Glowing circular rune disk with inner pulsing radius for ground AoE spells (`Veer Maze`, `Willow Ward`, `Maw Vortex`).
...
- **Bounty Values:**
  ...
  - Jungle Creep (Hollow / Veer camp): 140 XP / 75 Gold
  - Epic Boss (Treant / Maw): 500 XP / 250 Gold
...
- **Agimat Shop Catalogue:**
  1. **Blade of Searing Flame (1200g):** +45 ATK, +15% Attack Speed, Burns target for 15 dmg/sec.
  ...
  4. **Argent's Crescent Relic (1400g):** +50 Ability Power, +20% Mana/Energy Regen.
  5. **Talon of the Wakwak (1300g):** +35 ATK, +18% Lifesteal on physical strikes.
  6. **Idol Heart Ward (1500g):** +400 HP, Grants 200 HP shield when falling below 30% health.
...
### Phase 13: Lane AI Bot Heroes (Hollow / Sever Opponents)
...
- **AI State Machine:**
  ...
  4. **Jungle Objective State:** Joins team to contest Treant or Maw when pit combat is initiated.
